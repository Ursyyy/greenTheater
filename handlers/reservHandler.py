...
async def ReservHandler(cd: types.CallbackQuery, state: FSMContext):
	if cd.data.startswith('check_date'):
		...
		dates = getDays()
		for item in dates:
			keyboard.add(types.InlineKeyboardButton(text=item['dayName'], callback_data=f"check_date={item['dayStamp']}={isReserv}"))
		...
		if isReserv:
			...
			if reversId >= 0:
				await bot.edit_message_text(chat_id=cd.from_user.id,message_id=cd.message.message_id, text=f"Вжууух! Время коworkать! Бронь успешно создана. А значит, рабочее open-air место {getDay(date)} с {startTime}:00 по {endTime}:00 ждет тебя в Зелёном театре.")
